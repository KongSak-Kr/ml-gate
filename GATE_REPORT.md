...
- **#4 busy 중 저장** → **유효(존치)**: 멀티페이지 OCR/번역/인페인트 진행 중에도 SaveControls 가 활성(`disabled = !hasPage || isSaving` 만, `isBusy` 가드 없음) → 중간 스냅샷을 "성공"으로 영속(부분 처리 + 빈 undo). span 무관이라 R2 로 해소 안 됨. 제안 = busy 중 SaveControls writer-lock(또는 저장 시 busy 확인 다이얼로그).
